...
class CustomerController extends Controller
{
    public function __construct(){
    	$this->middleware('customer-api:api', ['except' => ['login','register'] ]);
    }
...
public function login(Request $request){
    ...
    try{
        ...
        if (! $token = auth('customer-api')->attempt($validator->validated())) {
                return response()->json(['error' => 'invalid login credentials'], 400);
            }

        $token_validity = 8760 * 60;

        auth('customer-api')->factory()->setTTL($token_validity);
        ...
        ->select('*')
        ->where('customers.email', $request->get('email'))
        ->get(); 
        $manage = (object)$token;
        ...
        return response()->json([
            ...
            'token' => $token,
            'token_validity'  => auth('customer-api')->factory()->getTTL() * 8760
        ]);
...
public function register(Request $request){
    ...
    return response()->json(['user'=>$user, 'message'=>'Customer Created Successfully'], 200);
}
...
{
    $userid = auth('customer-api')->setToken($request->bearerToken())->user()->id;
    $pp = auth('customer-api')->setToken($request->bearerToken())->user()->password;
